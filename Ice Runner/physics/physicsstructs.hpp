...
public:
    ...
    void AddWall(const AABB& wall)
    {
        ...
        else if (screenQuads_[1].Intersects(wall)) wallQuads_[1].push_back(wall);
        else if (screenQuads_[2].Intersects(wall)) wallQuads_[2].push_back(wall);
        else if (screenQuads_[3].Intersects(wall)) wallQuads_[3].push_back(wall);
        else assert(false && "wall doesn't fit in a quad?");
    }
