...
void MapTestingState::Enter()
{
    ...
    hadSuccessfullConfigure_ = loader.Configure();
    if(!hadSuccessfullConfigure_)
        std::cout << loader.GetError() << std::endl;

    hadSuccessfullMapLoading_ = loader.LoadMaps();
    if (!hadSuccessfullMapLoading_)
        std::cout << loader.GetError() << std::endl;

    if (hadSuccessfullConfigure_ && hadSuccessfullMapLoading_)
    {
...
GameState* MapTestingState::UpdateInMenu()
{
    ...
    sound::Effect* pEffect = resourceManager_.GetSoundEffect(ResourceManager::SoundEffect::MenuCycle);
...
void MapTestingState::UpdateOutOfMenu()
{
    ...
    if (player1State.IsDown(events::MetaEvent::eBack) || pCurrentPhysicsScene_->HitGoal())
    {
        ...
        if (pCurrentPhysicsScene_->HitGoal())
            resourceManager_.GetSoundEffect(ResourceManager::SoundEffect::Goal)->Play();
    ...
    if (!player.WasStopped())
        resourceManager_.GetSoundEffect(ResourceManager::SoundEffect::Bump)->Play();
...
void MapTestingState::NextMapListener(void* pTestingState)
{
    ...
    pState->resourceManager_.GetSoundEffect(ResourceManager::SoundEffect::MenuCycle)->Play();
}
...
void MapTestingState::TestMapListener(void* pTestingState)
{
    ...
    MapTestingState* pState = (MapTestingState*)pTestingState;
    if (!pState->hadSuccessfullConfigure_ || (pState->deckLoadingFuture_.valid() && !pState->deckLoadingFuture_.get()))
    {
        printf("Unable to configure the deck loader: %s\n Please fix the error and reconfigure\n", DeckLoader::GetInstance().GetError().c_str());
        return;
    }
    if (!pState->hadSuccessfullMapLoading_ || (pState->mapLoadingFuture_.valid() && !pState->mapLoadingFuture_.get()))
    {
    ...
    pState->resourceManager_.GetSoundEffect(ResourceManager::SoundEffect::MenuSelect)->Play();
}
...
void MapTestingState::ConfigureListener(void* pTestingState)
{
    ...
    pState->deckLoadingFuture_ = std::async(std::launch::async, [&loader, pState]()
    {
        ...
        pState->hadSuccessfullConfigure_ = result;
...
void MapTestingState::LoadMapsListener(void* pTestingState)
{
    ...
    pState->mapLoadingFuture_ = std::async(std::launch::async, [&loader, pState]()
    {
        ...
        if (result) pState->CreateMapResources();
        pState->loadingField_->ClearRenderDescription();
        pState->hadSuccessfullMapLoading_ = result;
        return result;
    });
}
